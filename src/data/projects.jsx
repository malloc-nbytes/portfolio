import ForgeDoc from '../components/ForgeDoc';
import EarlDoc from '../components/EarlDoc';

const iota = (function () {
    let i = 1;
    return function () {
        return i++;
    };
})();

export const projects = [
    {
        id: iota(),
        title: 'forge',
        desc: 'Forge your system',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/forge/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/forge',
        format: 'md',
        clone: '# NOTE: MAJOR CHANGES HAVE BEEN MADE, THIS PAGE IS NOT UP-TO-DATE NOW, WILL UPDATE IN THE FUTURE\ngit clone https://www.github.com/malloc-nbytes/forge.git\ncd forge\n./bootstrap.sh\nmake\nsudo make install\nsudo forge',
        forge: 'N/A',
        doc: ForgeDoc,
        tech: "C autotools",
        os: "Linux",
        cat: "package manager, LFS",
    },
    {
        id: iota(),
        title: 'EARL',
        desc: 'Evaluate And Run Language',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/earl/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/earl',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/earl.git\ncd earl\nmkdir build\ncd build\ncmake -S .. -B .\nmake\nsudo make install',
        forge: 'forge --only install malloc-nbytes@earl',
        doc: EarlDoc,
        tech: "C++ EARL CMake",
        os: "Linux OSX Windows",
        cat: "language",
    },
    {
        id: iota(),
        title: 'ww',
        desc: 'A Dependency-Free Terminal Text Editor',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/ww/main/README.org',
        repoUrl: 'https://github.com/malloc-nbytes/ww',
        format: 'org',
        clone: 'git clone https://www.github.com/malloc-nbytes/ww\ncd ww/src\npython3 premake.py --prefix=/usr/local\nmake\nsudo make install',
        forge: 'forge --only install malloc-nbytes@ww',
        doc: undefined,
        tech: "C",
        os: "Linux",
        cat: "editor",
    },
    {
        id: iota(),
        title: 'earlmgr',
        desc: 'The manager application for EARL',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/earlmgr/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/earlmgr',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/earlmgr.git\ncd earlmgr/src\nearl ./earlmgr.rl',
        forge: 'N/A',
        doc: undefined,
        tech: "EARL",
        os: "Linux OSX",
        cat: "utility, package manager",
    },
    {
        id: iota(),
        title: 'ampire',
        desc: 'A Terminal-based Music Player',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/ampire/main/README.org',
        repoUrl: 'https://github.com/malloc-nbytes/ampire',
        format: 'org',
        clone: 'git clone https://www.github.com/malloc-nbytes/ampire.git --recursive\ncd ampire\nmkdir build\ncd build\ncmake -S .. -B .\nmake\nsudo make install',
        forge: 'forge --only install malloc-nbytes@ampire',
        doc: undefined,
        tech: "C SDL autotools",
        os: "Linux OSX",
        cat: "music, TUI",
    },
    {
        id: iota(),
        title: 'AnimX',
        desc: 'Animated wallpapers for X',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/AnimX/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/AnimX',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/AnimX.git\ncd AnimX\nautoreconf --install\n./configure\nmake\nsudo make install',
        forge: 'forge --only install malloc-nbytes@AnimX',
        doc: undefined,
        tech: "C ffmpeg autotools",
        os: "Linux(X11)",
        cat: "image/video",
    },
    {
        id: iota(),
        title: 'xkillr',
        desc: 'Kill processes',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/xkillr/main/README',
        repoUrl: 'https://github.com/malloc-nbytes/xkillr',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/xkillr.git\ncd xkillr\nautoreconf --install\nmkdir build\ncd build\n../configure\nmake\nsudo make install',
        forge: 'forge --only install malloc-nbytes@xkillr',
        doc: undefined,
        tech: "C ncurses autotools",
        os: "Linux",
        cat: "utility, TUI",
    },
    {
        id: iota(),
        title: 'bless',
        desc: 'A "better" version of `less`',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/bless/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/bless',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/bless.git\ncd bless\nmkdir build\ncd build\ncmake -S .. -B .\nmake\nsudo make install',
        forge: 'N/A',
        doc: undefined,
        tech: "C CMake",
        os: "Linux OSX",
        cat: "utility, TUI",
    },
    {
        id: iota(),
        title: 'bm',
        desc: 'Bookmark Directories in the Terminal',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/bm/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/bm',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/bm.git\ncd bm\nmkdir build\ncd build\ncmake -S .. -B .\nmake\nsudo make install',
        forge: 'forge --only install malloc-nbytes@bm',
        doc: undefined,
        tech: "C CMake",
        os: "Linux (X11)",
        cat: "utility, TUI",
    },
    {
        id: iota(),
        title: 'forge-modules',
        desc: 'Known C modules for forge',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/forge-modules/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/forge-modules',
        format: 'md',
        clone: 'forge -r add-repo https://github.com/malloc-nbytes/forge-modules.git',
        forge: 'N/A',
        doc: undefined,
        tech: "C forge (automated)",
        os: "Linux",
        cat: "repository",
    },
    {
        id: iota(),
        title: 'sic',
        desc: 'Sample Input Creator',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/sic/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/sic',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/sic.git\ncd sic/src/\n./build.sh\n./install.sh',
        forge: 'N/A',
        doc: undefined,
        tech: "Haskell",
        os: "Linux",
        cat: "language",
    },
    {
        id: iota(),
        title: 'camelCase_to_snake',
        desc: 'Kill those camels!',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/camelCase_to_snake/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/camelCase_to_snake',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/camelCase_to_snake.git\ncd camelCase_to_snake/src\n./build.sh\n./install.sh',
        forge: 'N/A',
        doc: undefined,
        tech: "OCaml",
        os: "Linux OSX",
        cat: "joke",
    },
    {
        id: iota(),
        title: 'stt',
        desc: 'Simple Time Tracker',
        readmeUrl: 'https://raw.githubusercontent.com/malloc-nbytes/stt/main/README.md',
        repoUrl: 'https://github.com/malloc-nbytes/stt',
        format: 'md',
        clone: 'git clone https://www.github.com/malloc-nbytes/stt.git\ncd stt\ncargo build --release\n sudo cp ./target/release/stt /usr/local/bin',
        forge: 'N/A',
        doc: undefined,
        tech: "Rust",
        os: "Linux OSX",
        cat: "utility",
    },
];
